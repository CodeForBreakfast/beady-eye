...
use terminal::{over_the_described_subtree, window_over, THE_FIRST_BEAD, THE_TREES_HEADER};
...
/// The first bead of the capture's tree. `window_over` reads the id off the
/// border title whole, so this is not a prefix of the tree's header below it
/// the way a search over the stream would make it.
const THE_FIRST_BEAD_OF_THE_TREE: &str = "dun-0tp.6";

/// The tree's header, which is the row the walk leaves the selection on.
const THE_TREE: &str = "dun-0tp";
...
fn a_click_on_a_beads_row_selects_the_bead_drawn_there() {
    assert_eq!(
        the_window_a_click_opens("clicked-bead", THE_FIRST_BEAD).as_deref(),
        Some(THE_FIRST_BEAD_OF_THE_TREE),
...
fn a_click_on_the_row_the_selection_rests_on_opens_that_row() {
    assert_eq!(
        the_window_a_click_opens("clicked-header", THE_TREES_HEADER).as_deref(),
        Some(THE_TREE),
...
/// selection is on now, and say which bead the window that opened is over.
///
...
#[track_caller]
fn the_window_a_click_opens(named: &str, row: u16) -> Option<String> {
