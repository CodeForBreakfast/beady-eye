...
/// tree's header is `dun-0tp`, so a needle of the id alone would also be met
/// by the window this test exists to prove was *not* opened.
const THE_FIRST_BEADS_WINDOW: &str = "dun-0tp.6 · Esc to go back";

/// The same for the tree's header, which is the row the walk leaves the
/// selection on.
const THE_TREES_WINDOW: &str = "dun-0tp · Esc to go back";
